...
import type { AuditorFinding, LeadPosition } from '@platform/contracts';
import {
  useCrossings,
  useRationale,
  useLeadPositions,
 ...
 * legacy workbench tabs). Per selected crossing it renders the org's output:
 * the rationale draft, the ≤2 lead positions (Compliance / Risk, each with its
 * stance), and the auditor's findings. Retractions and advisory flags are
 * rendered VISUALLY DISTINCT so "ungrounded claim" is never conflated with
 * "debatable judgment" (Appendix A §A.4).
 */
export function OrgPanel({ intentId, crossingId }: OrgPanelProps) {
  ...
  return (
    <section aria-label="Agent org" className="rounded-lg border border-slate-800 bg-slate-900 p-4">
      ...
      {selected && <OrgCrossingView intentId={intentId} crossingId={selected} />}
    </section>
  );
}
...
function LeadPositionCard({ position }: { position: LeadPosition }) {
...
function OrgCrossingView({ intentId, crossingId }: { intentId: string; crossingId: string }) {
      ...
      <div>
        ...
        {ordered.length === 0 ? (
          ...
          <div className="space-y-2">
            {ordered.map((p) => (
              <LeadPositionCard key={p.lead} position={p} />
            ))}
          </div>
        )}
      </div>
