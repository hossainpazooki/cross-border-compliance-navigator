...
│   ├── treasury-settlement/ # ATLAS treasury loop Stage C(b): decision agent + settlement consumer (see Notes)
...
- `app/atlas/verify/route.ts` — `POST /atlas/verify` (same-origin proxy that forwards to `ke serve`'s `/verify`, since `ke serve` sends no CORS headers; see ATLAS provenance)
- `app/treasury/declare/route.ts` — `POST /treasury/declare` (decision agent); `app/treasury/reconcile/route.ts` — `GET /treasury/reconcile` (settlement consumer poll; the `vercel.json` cron target); `app/treasury/settlements/route.ts` — `GET /treasury/settlements` (ledger view). All 503 unless `TREASURY_GATE_URL` is set (see Notes → Treasury settlement).
...
**REST** — base `NEXT_PUBLIC_API_BASE_URL` (default **same-origin** `''` so a deployed app calls its own in-tree handlers with no per-deploy config; `dev:web`/`.env.local` set `http://localhost:8787` for the standalone reference backend; navigate/decoder/counterfactual fall back to in-app demo responses):
...
**WebSocket** — base `NEXT_PUBLIC_WS_BASE_URL` (unset ⇒ no WS; the stream falls back to SSE same-origin — the Vercel default), path `/v2/ws/trade/{intent_id}`:
...
- **ATLAS provenance (consumer-only).** `src/shared/atlas/provenance.ts` is a typed loader over the committed snapshot `src/shared/config/atlas-provenance.json` (refreshed by `npm run sync:atlas`). DeskHome's rule-pack provenance card joins regime → artifacts via `provenanceForRegime(regimeId)` and renders real signed-artifact metadata. Honesty boundaries are load-bearing and enforced in the UI copy: provenance is **surfaced, not re-verified**; signatures use fixed-seed TEST keys (`test-fixed-seed-1`), not production keys; `registry_state` is always `'unknown'` (live Published/Revoked lifecycle lives only in the ATLAS registry, `ke-cli serve`, Gate 5). COMPASS does not compile/sign/attest/publish. The snapshot pins `atlas_commit` and `platform_corpus_commit`.
- **Treasury settlement (Stage C(b), built 2026-07-12).** `src/features/treasury-settlement/` closes COMPASS's side of the ATLAS treasury intent loop: a **decision agent** (`decisionAgent.ts` — resolves the pinned signed IntentSpec hash via `provenanceForRegime('treasury_payments_v1')`, declares to the Go gate at `TREASURY_GATE_URL`, holds NO dispatch handle; the sync response is observation only) and a **settlement consumer** (`reconcile.ts` — pulls the gate's durable `GET /v2/events` by cursor and recomputes settlements from OBSERVED `ACHIEVED` records only; at-most-once via the keyed `settlementLedger.ts`, settlements written before the cursor advances). The gate wire types in `types.ts` mirror `treasury-intent-controller` verbatim — never rename fields. Honesty boundaries: `FileSettlementLedger` (`TREASURY_LEDGER_FILE`, default `.treasury/ledger.json`) is **local-durable only** — on Vercel a KV/Postgres adapter is *planned* and required before the cron (`vercel.json` → `/treasury/reconcile`, every 5 min) can claim durability in production; the agent's criteria/threshold config (`GOLDEN_PAYMENT_CONFIG`) is app config bound to the pinned spec hash — extracting them from the IntentSpec payload is the ATLAS resolver-extraction slice (recorded parity debt).
- **Live verification (wired, dormant by default).** A fail-closed verification layer (ADR-0019) is now in-tree under `src/shared/atlas/` — pure `decideGate` (allowed only when crypto-`verified` AND `registry_state==='Published'`; `Revoked`/`Deprecated`/`Unknown`/`not_found`/rejected ⇒ blocked), a `ke serve` client + gateway + `useArtifactVerification` hook, and the `VerificationChip` UI. It is **off unless `NEXT_PUBLIC_USE_WASM_VERIFY==='true'`** (default ⇒ snapshot mode, chip reads "Not verified (snapshot)"). Two paths: HTTP `serve` (the active one — `POST /atlas/verify` same-origin proxy → `ke serve`, since `ke serve` has no CORS) selected when `NEXT_PUBLIC_ATLAS_REGISTRY_URL` is set; and an in-browser WASM path via a **dependency-injected** adapter (`wasmVerifier.ts`) that imports nothing from `@platform/atlas-artifact` (unpublished) — wiring its real import is the one gated step. No `@platform/atlas-artifact` dependency is in `package.json`. A browser **green** "Verified — Published" render additionally needs registry/snapshot hash alignment (the snapshot's `gen-fixtures` goldens aren't registry-publishable) — see `dev/briefs/atlas-live-verify-session.md`.
